...
# database_url = "postgresql+pygresql://{}:{}@{}:{}/housinghub".format(os.environ['DB_USER'],os.environ['DB_PASSWORD'],os.environ['DB_HOST'],5432)
...
# # global config
# token_secret = "{}-{}".format(secrets.token_hex(64), time.time())
# token_rotation_interval_sec = int(
#     os.environ.get("TOKEN_SECRET_KEY_ROTATION_SECONDS") or 172800)
# token_future_rotation_date = datetime.now(
#     timezone.utc) + timedelta(seconds=token_rotation_interval_sec)
# token_exp_seconds = 10800
# token_encrypt_alg = 'HS256'
# try:
#     token_exp_seconds = int(os.environ['TOKEN_EXP_SECONDS'])
# except KeyError:
#     logging.debug(
#         "TOKEN_EXP_SECONDS not set in environment, defaulting to {} seconds".
#         format(token_exp_seconds))
...
#         additional validate_incoming_request and add request and jwt arguments
#     """
#     if (endpt.get("path").startswith("/auth/")):
#         return func
#     # else return function with wrapper
#     def wrapper():
#         (uInfo, err) = auth.validate_incoming_request(request)
#         if err is not None:
...
# for endpt in supported_crud_endpoints:
